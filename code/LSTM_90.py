...
# 在进行 fit_transform 之前，确保 train_df 和 test_df 的列顺序一致
# 并且 scaler.n_features_in_ 会被正确设置
...
X_train, y_train = create_sequences(train_scaled, target_idx=target_feature_index)
X_test, y_test = create_sequences(test_scaled, target_idx=target_feature_index)
...
model = LSTMModel(input_size=X_train.shape[2], hidden_size=128, num_layers=4, output_len=90).to(device)
...
model.eval()
all_preds_sliding_window = []  # 用于存储每个滑动窗口的第一个预测值
all_trues_sliding_window = []  # 用于存储每个滑动窗口对应的真实值

input_len = 90
start_index_for_prediction = input_len

with torch.no_grad():
    # 循环遍历测试集中每个可以进行预测的日期
    for i in range(start_index_for_prediction, len(test_scaled)):
        current_input_window = test_scaled[i - input_len: i]
...
plt.xlabel("Days in Test Set (Starting from Day 91)")
